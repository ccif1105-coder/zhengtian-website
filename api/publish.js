...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [ref, insights, sitemap, llms] = await Promise.all([
    ...
    if (!insights.includes(cardMarker) || !sitemap.includes(sitemapMarker) || !llms.includes(llmsMarker)) throw new Error("网站缺少自动发布标记，请先更新网站模板。 ");
    ...
    const [articleSha, insightsSha, sitemapSha, llmsSha, coverSha] = await Promise.all([
